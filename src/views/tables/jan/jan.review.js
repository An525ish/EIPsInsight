...
function janReview(props) {
  const [info, setInfo] = useState()
  const [review, setReview] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setReview(JSON.parse(localStorage.getItem('janReview')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('janReview', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('janReview', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'janReview') {
      return true
    } else {
      return false
    }
