...
function janStagnant(props) {
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('janStagnant', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('janStagnant', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'janStagnant') {
      return true
    } else {
      return false
    }
