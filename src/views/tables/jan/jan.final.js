...
function janFinal(props) {
  const [info, setInfo] = useState()
  const [final, setFinal] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setFinal(JSON.parse(localStorage.getItem('janFinal')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('janFinal', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('janFinal', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'janFinal') {
      return true
    } else {
      return false
    }
