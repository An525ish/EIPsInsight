...
function janLastCall(props) {
  const [info, setInfo] = useState()
  const [lastCall, setLastCall] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setLastCall(JSON.parse(localStorage.getItem('janLastCall')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('janLastCall', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('janLastCall', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])
