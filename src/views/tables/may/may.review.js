...
function mayReview(props) {
  const [info, setInfo] = useState()
  const [review, setReview] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setReview(JSON.parse(localStorage.getItem('mayReview')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('mayReview', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('mayReview', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'mayReview') {
      return true
    } else {
      return false
    }
