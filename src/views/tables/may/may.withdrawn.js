...
function mayWithdrawn(props) {
  const [info, setInfo] = useState()
  const [Withdrawn, setWithdrawn] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setWithdrawn(JSON.parse(localStorage.getItem('mayWithdrawn')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('mayWithdrawn', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('mayWithdrawn', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'mayWithdrawn') {
      return true
    } else {
      return false
    }
