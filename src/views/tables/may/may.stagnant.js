...
function mayStagnant(props) {
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('mayStagnant', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('mayStagnant', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'mayStagnant') {
      return true
    } else {
      return false
    }
