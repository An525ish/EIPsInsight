...
function mayLastCall(props) {
  const [info, setInfo] = useState()
  const [lastCall, setLastCall] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setLastCall(JSON.parse(localStorage.getItem('mayLastCall')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('mayLastCall', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('mayLastCall', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'mayLastCall') {
      return true
    } else {
      return false
    }
