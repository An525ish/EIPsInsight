...
function marchReview(props) {
  const [info, setInfo] = useState()
  const [review, setReview] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setReview(JSON.parse(localStorage.getItem('marchReview')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('marchReview', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('marchReview', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])
