...
function marchWithdrawn(props) {
  const [info, setInfo] = useState()
  const [Withdrawn, setWithdrawn] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setWithdrawn(JSON.parse(localStorage.getItem('marchWithdrawn')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('marchWithdrawn', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('marchWithdrawn', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'marchWithdrawn') {
      return true
    } else {
      return false
    }
