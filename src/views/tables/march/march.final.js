...
function marchFinal(props) {
  const [info, setInfo] = useState()
  const [final, setFinal] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setFinal(JSON.parse(localStorage.getItem('marchFinal')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('marchFinal', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('marchFinal', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])
