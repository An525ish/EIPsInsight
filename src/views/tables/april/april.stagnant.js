...
function aprilStagnant(props) {
  const [info, setInfo] = useState()
  const [Stagnant, setStagnant] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setStagnant(JSON.parse(localStorage.getItem('aprilStagnant')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('aprilStagnant', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('aprilStagnant', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'aprilStagnant') {
      return true
    } else {
      return false
    }
