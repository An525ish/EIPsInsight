...
function aprilFinal(props) {
  const [info, setInfo] = useState()
  const [final, setFinal] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setFinal(JSON.parse(localStorage.getItem('aprilFinal')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('aprilFinal', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('aprilFinal', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'aprilFinal') {
      return true
    } else {
      return false
    }
