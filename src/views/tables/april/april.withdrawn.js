...
function aprilWithdrawn(props) {
  const [info, setInfo] = useState()
  const [Withdrawn, setWithdrawn] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setWithdrawn(JSON.parse(localStorage.getItem('aprilWithdrawn')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('aprilWithdrawn', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('aprilWithdrawn', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])
