...
function aprilDraft(props) {
  const [info, setInfo] = useState()
  const [draft, setDraft] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setDraft(JSON.parse(localStorage.getItem('aprilDraft')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('aprilDraft', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('aprilDraft', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'aprilDraft') {
      return true
    } else {
      return false
    }
