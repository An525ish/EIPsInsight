...
function febWithdrawn(props) {
  const [info, setInfo] = useState()
  const [Withdrawn, setWithdrawn] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setWithdrawn(JSON.parse(localStorage.getItem('febWithdrawn')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('febWithdrawn', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('febWithdrawn', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'febWithdrawn') {
      return true
    } else {
      return false
    }
