...
function febLastCall(props) {
  const [info, setInfo] = useState()
  const [lastCall, setLastCall] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setLastCall(JSON.parse(localStorage.getItem('febLastCall')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('febLastCall', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('febLastCall', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'febLastCall') {
      return true
    } else {
      return false
    }
