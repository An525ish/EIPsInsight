...
function febStagnant(props) {
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('febStagnant', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('febStagnant', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'febStagnant') {
      return true
    } else {
      return false
    }
