...
function febReview(props) {
  const [info, setInfo] = useState()
  const [review, setReview] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setReview(JSON.parse(localStorage.getItem('febReview')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('febReview', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('febReview', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'febReview') {
      return true
    } else {
      return false
    }
