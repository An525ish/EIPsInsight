...
function febDraft(props) {
  const [info, setInfo] = useState()
  const [draft, setDraft] = useState()

  useEffect(() => {
    setInfo(JSON.parse(localStorage.getItem('count')))
    setDraft(JSON.parse(localStorage.getItem('febDraft')))
  }, [])
  ...
  useEffect(() => {
    if (props.data !== undefined) {
      ...
      localStorage.setItem('febDraft', JSON.stringify(props.data.filter(filterDraft)))
    } else {
      localStorage.setItem('count', JSON.stringify(info))
      localStorage.setItem('febDraft', JSON.stringify(info.filter(filterDraft)))
    }
  }, [info])

  function filterDraft(item, index) {
    if (index >= 130 && item[1] === 'febDraft') {
      return true
    } else {
      return false
    }
